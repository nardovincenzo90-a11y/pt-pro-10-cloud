...
async function smartDay(uid,plan,date,dayType='AUTO'){
 ...
 let foods=await api.get('foods',{select:'id,name,category,department,kcal_100,protein_100,carbs_100,fat_100,default_unit,active',active:'eq.true',limit:1000}).catch(()=>[]);if(!foods.length)foods=(window.PTPRONutritionLibrary?.foods||coreFoods).map(x=>({...x,core_id:x.id||x.core_id,id:null,default_unit:x.default_unit||'g'}));let profile={};try{profile=JSON.parse(localStorage.getItem('ptpro_nutrition_profile')||'{}')}catch{}if(profile.mode)foods=foods.filter(x=>!x.diet_modes||x.diet_modes.includes(profile.mode));if(profile.avoid?.length)foods=foods.filter(x=>!profile.avoid.some(a=>`${x.name} ${(x.allergens||[]).join(' ')}`.toLowerCase().includes(String(a).toLowerCase())));if(!foods.length)foods=coreFoods;const g=classify(foods),mealNames=['Colazione','Pranzo','Spuntino','Cena'],mealTags=['breakfast','lunch','snack','dinner'],ratios=[.24,.34,.16,.26],seed=hash(date);
